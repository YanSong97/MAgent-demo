...
models = {'red': DQN_IL(obs_dim = env.observation_spaces['red_0'], action_space=env.action_spaces['red_0']),
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        #decision making
        action_dict = {}
        current_key_list = env.agents   #list of agents that are still alive
        for agent in current_key_list:
...
##############################################  Recording data   ##############################################
        ...
        for key, value in reward_dict.items():
            if 'red' in key:
                ...
                replay_memories['red'].push(state[key], action_dict[key], reward_dict[key], next_state[key],
                                            float(not done_dict[key]))
                ...
                if not args.single_handle:
                    replay_memories['blue'].push(state[key], action_dict[key], reward_dict[key], next_state[key],
                                              float(not done_dict[key]))
...
############################################## Model training   ##############################################
        if len(replay_memories['red']) > args.batch_size and total_numsteps % args.train_interval == 0:
            if args.single_handle:
                red_temp_loss = 0
            else:
            ...
            for _ in range(args.train_per_step):
                ...
                red_agent_loss = models['red'].learn(replay_memories['red'], args.batch_size, updates)
                red_temp_loss += red_agent_loss

                if not args.single_handle:
                    blue_agent_loss = models['blue'].learn(replay_memories['blue'], args.batch_size, updates)
                    blue_temp_loss += blue_agent_loss


                if args.tensorboard:
                    writer.add_scalar('loss/train (red)', red_agent_loss, updates)
                    if not args.single_handle:
                        writer.add_scalar('loss/train (blue)', blue_agent_loss, updates)
    ...
    if i_episode % args.eval_interval == 0:
        ...
        if success_rate > max_success_rate and args.saving:
            ...
            save_key_list = ['red'] if args.single_handle else ['red', 'blue']

            save_models(save_model_path, models, save_key_list, updates, True)
            max_success_rate = success_rate
            #ax_killed = average_killed_blue
